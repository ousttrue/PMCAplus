...
int load_textures_list(TEXTURES textures[], unsigned short *textures_max,
                       FILE *list);
int load_materials_list(MATERIALS materials[], unsigned short *materials_max,
                        FILE *list);
int load_bone_group_list(BONE_GROUP_CONF bone_group[],
                         unsigned short *bone_group_max, FILE *list);
int load_rc();
...
int load_parts_list(PARTS parts[], unsigned short *parts_max, FILE *list) {
  ...
  if (strcmp(str, "PMCA Parts list v1.0") != 0)
    return max; // ファイルが指定の文字列で始まっているか
...
int load_textures_list(TEXTURES textures[], unsigned short *textures_max,
  ...
  printf("%s\n", str);
#endif
  if (strcmp(str, "PMCA Textures list v1.0") != 0)
    return 1; // ファイルが指定の文字列で始まっているか
  ...
  while (fgets(str, 256, list) != NULL) {
    ...
    if (strcmp(str, "texture") == 0) {
      ...
      for (; i < 16; i++) {
        ...
        // sscanf(str, "%s %s %s", textures[index].tex[i],
        // textures[index].tex_comment[i], textures[index].pic[i]);
      }
      /*
      #ifdef DEBUG
              printf("%d\n", max);
  ...
  テクスチャ名(char[16][21]) テクスチャコメント(char[16][32])
  ピクチャ名(char[16][96])
...
int load_materials_list(MATERIALS materials[], unsigned short *materials_max,
  ...
  if (strcmp(str, "PMCA Materials list v1.0") != 0 &&
      strcmp(str, "PMCA Materials list v1.1") != 0)
    return 1; // ファイルが指定の文字列で始まっているか
  ...
  while (ret != NULL) {
    if (strcmp(str, "material") == 0) {
...
#ifdef DEBUG
      printf("\n%d name:%s\ncomment:%s\n", max, materials[max].name,
             materials[max].comment);
#endif
      // 多重定義のチェック
      tmp = 0;
      for (i = 0; i < max; i++) {
      ...
      for (i = 0; i < 16; i++) {
        if (materials[index].mat[i].tex[0] == 0)
          break;
      }
      for (; i < 16; i++) {
        tmp = 0;
        ...
        // sscanf(str, "%s %s", materials[index].mat[i].tex,
        // materials[index].mat[i].comment, materials[index].mat[i].pic);
        ...
        if (p != NULL)
          *p = '\0';
        if (strcmp(str, "") == 0 || strcmp(str, "material") == 0)
          break;
        sscanf(str, "%f %f %f", &materials[index].mat[i].dif[0],
               &materials[index].mat[i].dif[1],
               &materials[index].mat[i].dif[2]);
        ...
        if (strcmp(str, "") == 0 || strcmp(str, "material") == 0)
          break;
        sscanf(str, "%f %f %f", &materials[index].mat[i].spec[0],
               &materials[index].mat[i].spec[1],
               &materials[index].mat[i].spec[2]);
        ...
        if (strcmp(str, "") == 0 || strcmp(str, "material") == 0)
          break;
        sscanf(str, "%f %f %f", &materials[index].mat[i].mir[0],
               &materials[index].mat[i].mir[1],
               &materials[index].mat[i].mir[2]);

#ifdef DEBUG
        printf("   %d %s %s\n", i, materials[index].mat[i].tex,
               materials[index].mat[i].comment);
#endif
      }
...
int load_bone_group_list(BONE_GROUP_CONF bone_group[],
  ...
  printf("%s\n", str);
#endif
  if (strcmp(str, "PMCA Bone_Group list v1.0") != 0)
    return 1; // ファイルが指定の文字列で始まっているか
        ...
        while(fgets(str, 1024, rc) != NULL){
                ...
                if(strcmp(buf, "btn_txt") == 0){
                        ...
                        *p = '\0';
                        ...
                        p = g_ui.welcome;
                        if(strstr(buf, "mato") == NULL || strstr(buf,
VERSION_NO) == NULL){ p = strchr(g_ui.welcome, '\0');
                        }
                        strcpy(p, buf);
                        ...
                        sscanf(p, "%f %f %f", &g_ui2.gl_col_bg[0],
&g_ui2.gl_col_bg[1], &g_ui2.gl_col_bg[2]); }else if(strcmp(buf, "gl_show") ==
0){ read_line(p, buf); if(strcmp(buf, "axis")  == 0)pc = &g_ui2.show_axis; else
if(strcmp(buf, "grid")  == 0)pc = &g_ui2.show_grid; else continue; if(strchr(p,
'x') != NULL)*pc = *pc | 0x01;	//全て加算して書いてある部分全てを1にする
